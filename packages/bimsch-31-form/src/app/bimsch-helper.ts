...
export function setSaveData(newValue?) {
  ...
  } else if (getField('Formular.Datenspeicherung').checked)
    getField('fgMetaData.saveData').value = '1';
  else getField('fgMetaData.saveData').value = '0';
  if (getField('fgMetaData.saveData').value == '1') {
    getField('Formular.Datenspeicherung').checked = true;
    bolShow('btnSave');
  } else {
    getField('Formular.Datenspeicherung').checked = false;
    bolHide('btnSave');
  }
}
...
function SendForm(submissionType) {
  ...
  getField('fgMetaData.contactFirstName').value =
    getField('account.firstName').value;
  getField('fgMetaData.contactPhone').value =
    getField('account.contactNr').value;
  ...
  document
    .getElementById('operator.address.federalState')
  ...
  bolDialog.ShowInfo(
    'Datenübertragung',
    'Die Daten des Formulars werden übertragen!'
  );
  bolSendForm('btnSend');
}
...
export function mapField(
  ...
  if (destinationFieldReadonly) getField(destinationFieldName).readonly = true;
  else getField(destinationFieldName).readonly = false;
}

/**** Mapping *************************************************************************************** */
export function mapTecFields(bolDialog: bolc__Dialog) {
  // create and set the link for data privacy proclamation
  setText2Label(
    'bol.txtDSGVOlink',
    '<a href="' +
      getField('fgMetaData.urlPortal').value +
      'de/datenschutz" target="_blank" rel="noreferrer,nofollow">Details zum Datenschutz</a>',
    bolDialog
  );

  // Update current date in field and label
  getField('Formular.Datum').value = bolGetDateD();
  setText2Label('bol.txtFormDate', getField('Formular.Datum').value, bolDialog);
  // read the value for the LEIKA nr and display
  setText2Label(
    'bol.txtLEIKANr',
    'LEIKA: ' + getField('fgMetaData.leikaNr').value,
    bolDialog
  );
  setText2Label('bol.txtFormVersion', _FormVersion, bolDialog);

  if (getField('fgMetaData.saveData').value == '1') setSaveData(true);
  ...
  function SetELSTERtest(mode) {
    if (mode) {
      ...
      getField('fgMetaData.elsterTestMsg').value =
        'Achtung! Login mit ELSTER Testzertifikat!';
      document.getElementById('bol.txtELSTER_Test').innerText = getField(
        'fgMetaData.elsterTestMsg'
      ).value;
    } else {
...
export function listGetShortText(listname) {
  ...
  if (lb.value == '') e.value = '';
    ...
    for (let i = 0; i < lb.options.length; i++)
      if (lb.options[i].value == lb.value) {
        e.value = lb.options[i].innerText;
        break;
      }
}
...
async function loadList_FederalStates(listname) {
  ...
  if (lb == undefined) return;
  listClearOptions(lb);
  for (let i = 0; i < md_FederalStates.length; i++)
    lb.options.add(listAddOption(md_FederalStates[i]['name']));
}
...
async function loadList_CombustionPlantTypes(listname) {
  ...
  if (lb == undefined) return;
  listClearOptions(lb);
  for (let i = 0; i < md_CombustionPlantTypes.length; i++)
    lb.options.add(listAddOption(md_CombustionPlantTypes[i]['shortText']));
}
...
async function loadList_Persons(listname) {
  ...
  for (let i = 0; i < md_Persons.length; i++) {
    lb.options.add(
      listAddOption(
        md_Persons[i]['id'],
        md_Persons[i]['firstName'] +
          ' ' +
          md_Persons[i]['lastName'] +
          ', ' +
          md_Persons[i]['funktion']
      )
    );
  }
}
...
function dlgPerson(fname) {
  ...
  s =
    s +
    '<div id="bolDialogHeader" class="modal-header bol-dialog-header"><h5 class="modal-title" id="bolDialogTitle"> Person ändern/auswählen</h5></div>';
  s = s + '<div class="modal-body" id="bolDialogMessage"><div class="row">';
  s = s + '<div class="col-12">';
  s =
    s +
    'Wählen Sie aus der Liste der bekannten Ansprechpersonen aus (Vorname Nachname, Funktion):';
  s = s + '</div><div class="col-12" style="margin-top: 10px">';
  s =
    s +
    '<select class="listbox form-control" id="dlgListContact" title="Ansprechpersonen" size="10"></select>';
  s = s + '</div></div>';
  s = s + '<div class="modal-footer">';
  // for the footer buttons set the name of the destination field and button
  s =
    s +
    '<button type="button" class="btn cc-button" data-dismiss="modal" data-bs-dismiss="modal" onclick="dlgPersonClose(`' +
    fname +
    '`);">übernehmen</button>';
  s =
    s +
    '<button type="button" class="btn cc-button" data-dismiss="modal" data-bs-dismiss="modal" onclick="dlgPersonCancel(`' +
    fname +
    '`);">abbrechen</button>';
  s = s + '</div></div></div>';
  e.innerHTML = s;
...
function dlgPersonCancel(fname) {
  {
    try {
      document.getElementById('btn.' + fname).focus();
    } catch (err) {}
  }
}
...
async function loadList_NACE(listname) {
  ...
  for (let i = 0; i < md_NACE.length; i++)
    lb.options.add(
      listAddOption(
        md_NACE[i]['code'],
        md_NACE[i]['code'] + ' - ' + md_NACE[i]['shortText']
      )
    );
}
/***************************************************************************************
...
export async function filterNACE(listname, filtername) {
  ...
  let o;
  o = md_NACE.filter(
    ({ shortText }) => shortText.toLowerCase().indexOf(fv.toLowerCase()) >= 0
  );
  if (o == undefined || o.length == 0)
    o = md_NACE.filter(({ code }) => code.indexOf(fv) >= 0);
  // ok, som,ething found, clear the listbox and refill with entries matching the filter criteria
  lb.innerHTML = '';
  for (let i = 0; i < o.length; i++)
    lb.options.add(
      listAddOption(o[i]['code'], o[i]['code'] + ' - ' + o[i]['shortText'])
    );
}
/***************************************************************************************
 * @name		dlgNACE
...
export async function dlgNACE(fname) {
  ...
  s =
    s +
    '<div id="bolDialogHeader" class="modal-header bol-dialog-header"><h5 class="modal-title" id="bolDialogTitle"> NACE-Code ändern/auswählen</h5></div>';
  s = s + '<div class="modal-body" id="bolDialogMessage"><div class="row">';
  s = s + '<div class="col-12">';
  s = s + '<label for="filter.NACE">filtern nach</label>';
  s =
    s +
    '<input id="filter.NACE" aria-describedby="filter.NACE-infield-error" class="form-control" type="text" name="filter.NACE" title="NACE filtern nach" onkeyup="filterNACE(`dlgListNACE`, `filter.NACE`);" value="" required=""><div class="infielderror" id="filter.NACE-infield-error"></div>';
  s = s + '</div><div class="col-12" style="margin-top: 10px">';
  s =
    s +
    '<select class="listbox form-control" id="dlgListNACE" title="NACE" size="10"></select>';
  s = s + '</div></div>';
  s = s + '<div class="modal-footer">';
  s =
    s +
    '<button type="button" class="btn cc-button" data-dismiss="modal" data-bs-dismiss="modal" onclick="dlgNACEClose(`' +
    fname +
    '`);">übernehmen</button>';
  s =
    s +
    '<button type="button" class="btn cc-button" data-dismiss="modal" data-bs-dismiss="modal" onclick="dlgNACECancel(`' +
    fname +
    '`);">abbrechen</button>';
  s = s + '</div></div></div>';
  e.innerHTML = s;
...
async function loadList_EmissionControlSystems(listname) {
  ...
  if (lb == undefined) return;
  listClearOptions(lb);
  for (let i = 0; i < md_EmissionControlSystems.length; i++)
    lb.options.add(
      listAddOption(
        md_EmissionControlSystems[i]['ident'],
        md_EmissionControlSystems[i]['shortText']
      )
    );
}
...
function dlgEmissionControlSystemsClose() {
  getField('plant.emissionControlSystem.codes').value = document.getElementById(
    'list_ArtAbgaseinrichtungCodes'
  ).value;
  getField('plant.emissionControlSystem.names').value = getField(
    'list_ArtAbgaseinrichtungNamen'
  ).value;
