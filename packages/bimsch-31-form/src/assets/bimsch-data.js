...
const BImSch_FormModeTypes = ["local", "portalNew", "portalOpen", "findform", "loaclSaved"];
let BImSch_FormMode = "local";
...
async function getFormData() {
    ...
    if (md_processNr != undefined && md_processNr != "") {
        ...
        {try {getField("fgMetaData.processId").value = md_FormData.processId} catch(err) {}}
    } else {
        // clear hidden fields
        getField("fgMetaData.processNr").value = "";
        getField("fgMetaData.fileName").value = "";
    }
    SetText2Label("bol.txtProcessNr", getField("fgMetaData.processNr").value);   
    if (location.href.indexOf("sim=1") >= 0) useSimulation = true;
    if (useSimulation) _local_DataForm();

    // for debugging
    console.log(BImSch_FormMode, useSimulation, md_URL_resolver, location.href);
}
 ...
 * @returns		nothing
***************************************************************************************/
async function svcData_Form(url) {
    // if already loaded exit
    if (md_FormData.length != 0) return;
    if (url == undefined || url == "") return;
    let response = await fetch(url, {'mode': 'cors','Content-Type': 'application/json' });
    // we get something, so load the stuff to md_FormData
    if (response.ok) {md_FormData = await response.json()} 
}
...
async function svcData_Authorities() {
    ...
    if (BImSch_FormMode == "local" || useSimulation) {_local_Authorities(); return}
    let url = getField("fgMetaData.urlCatalogs").value  + "authorities";
    let response = await fetch(url, {'mode': 'cors','Content-Type': 'application/json' });
    if (response.ok) {md_Authorities = await response.json()}
}
async function svcData_FederalStates() {
    if (md_FederalStates.length != 0) return;
    if (BImSch_FormMode == "local" || useSimulation) {_local_FederalStates(); return}
    let url = getField("fgMetaData.urlCatalogs").value  + "federal-states";
    let response = await fetch(url, {'mode': 'cors', 'Content-Type': 'application/json' });
    if (response.ok) {md_FederalStates = await response.json()} 
}
async function svcData_Fuels() {
    if (md_Fuels.length != 0) return;
    if (BImSch_FormMode == "local" || useSimulation) {_local_Fuels(); return}
    let url = getField("fgMetaData.urlCatalogs").value  + "fuels";
    let response = await fetch(url, {'mode': 'cors','Content-Type': 'application/json' });
    if (response.ok) {md_Fuels = await response.json()}
}
async function svcData_NACE() {
    if (md_NACE.length != 0) return;
    if (BImSch_FormMode == "local" || useSimulation) {_local_NACE(); return}
    let url = getField("fgMetaData.urlCatalogs").value  + "nace-codes";
    let response = await fetch(url, {'mode': 'cors','Content-Type': 'application/json' });
    if (response.ok) {md_NACE = await response.json()} 
}
async function svcData_PlantTypes() {
    if (md_PlantTypes.length != 0) return;
    if (BImSch_FormMode == "local" || useSimulation) {_local_PlantTypes(); return}
    let url = getField("fgMetaData.urlCatalogs").value  + "plant-types";
    let response = await fetch(url, {'mode': 'cors','Content-Type': 'application/json' });
    if (response.ok) {md_PlantTypes = await response.json()}
}
async function svcData_CombustionPlantTypes() {
    if (md_CombustionPlantTypes.length != 0) return;
    if (BImSch_FormMode == "local" || useSimulation) {_local_CombustionPlantTypes(); return}
    let url = getField("fgMetaData.urlCatalogs").value  + "combustion-plant-types";      
    let response = await fetch(url, {'mode': 'cors','Content-Type': 'application/json' });
    if (response.ok) {md_CombustionPlantTypes = await response.json()} 
}
async function svcData_EmissionControlSystems() {
    if (md_EmissionControlSystems.length != 0) return;
    if (BImSch_FormMode == "local" || useSimulation) {_local_EmissionControlSystems(); return}
    let url = getField("fgMetaData.urlCatalogs").value  + "emission-control-systems";
    let response = await fetch(url, {'mode': 'cors','Content-Type': 'application/json' });
    if (response.ok) {md_EmissionControlSystems = await response.json()}
}
...
function _local_DataForm() {
    md_FormData = {"account":{"id":2,"firstName":"Ulrich","lastName":"Fraus","email":"ulrich.fraus@bol-systemhaus.de","contactNr":"089 21 09 67 20","elsterTest":false},
    "operator":{"id":2,"name":"bol Behörden Online Systemhaus GmbH","address":{"id":2,"street":"Einsteinstr.","houseNr":"14","zipCode":"85716","city":"Unterschleißheim","federalState":"BW","district":"München"},
        "contactPerson":{"id":2,"firstName":"Ulrich","lastName":"Fraus","funktion":"GF","phone":"089 21 09 67 20","email":"ulrich.fraus@bol-systemhaus.de"},"elsterTest":false},
    "operatingSite":{"name":"Test bol","nr":"123456","naceCode":{"id":361,"code":"37000","shortText":"Abwasserentsorgung","longText":"37000 - Abwasserentsorgung"},"building":null,
        "address":{"id":2,"street":"Einsteinstr.","houseNr":"14","zipCode":"85716","city":"Unterschleißheim","federalState":"BY","district":"München"},
        "contactPerson":{"id":2,"firstName":"Ulrich","lastName":"Fraus","funktion":"GF","phone":"089 21 09 67 20","email":"ulrich.fraus@bol-systemhaus.de"},
        "competentAuthority":{"id":2,"ident":"RP 20","shortText":"SGD Süd"},
        "siteAccordingTo12BImSchV":false},
    "plants":[
        {"plantType":"1","name":"Formularschmiede","nr":"233",
            "address":{"id":2,"street":"Einsteinstr.","houseNr":"14","zipCode":"85716","city":"Unterschleißheim","federalState":"BY","district":"München"},
            "contactPerson":{"id":3,"firstName":"Dirk","lastName":"Werther","funktion":"Vertrieb","phone":"+49 (89) 2109 67-25","email":"dirk.werther@bol-systemhaus.de"},
            "plantSection":"Hauptanlage","requiresBImSchGApproval":true,"approvalReferenceNumber":"AZ1","nrAccordingToAppendix1Of4BImSchV":"AZ2","nrAccordingToIEDirective":"AZ3","nrAccordingToPRTR":"AZ4",
            "outdoorPlant":true,"eastValue":12345,"northValue":5678,"district":"ycvxd s","corridor":"cdsfgb","parcel":"cdsafvghbn",
            "operationalUnit":"Schreibtisch","commissioningDate":"12.12.2022",
                "competentAuthority":{"id":1,"ident":"RP 10","shortText":"SGD Nord"},
            "operatingSiteId":1,
                "pollutionSources":[{"nr":"SH1","name":"Schornstein 1","eastValue":"1234","northValue":"567","geometricHeight":"43"}]}
            ],
    "contactPersons":[{"id":2,"firstName":"Ulrich","lastName":"Fraus","funktion":"GF","phone":"089 21 09 67 20","email":"ulrich.fraus@bol-systemhaus.de"},
        {"id":3,"firstName":"Dirk","lastName":"Werther","funktion":"Vertrieb","phone":"+49 (89) 2109 67-25","email":"dirk.werther@bol-systemhaus.de"}]};
}
...
function _local_Persons() {
    md_Persons = [
        {"id": 23456, "lastname": "Schmidt", "firstname": "Julia", "funktion": "Abteilungsleiter Produktion", "phone": "+49 40 50 27 02", "email": "Julia.Schmidt@email.com"},
        {"id": 33884, "lastname": "Mustermann", "firstname": "Max", "funktion": "Vorstand", "phone": "+49 (1234) 5678", "email": "mustermann@email.com"},
        {"id": 55441, "lastname": "Müller", "firstname": "Hans", "funktion": "Anlagenfahrer", "phone": "+49 1234-5678", "email": ""}
    ];
}
function _local_Authorities() {
    md_Authorities = [
        {"id": 1, "ident": "RP 10", "shortText": "Struktur- und Genehmigungsdirektion Nord"},
        {"id": 2, "ident": "RP 12", "shortText": "Struktur- und Genehmigungsdirektion Süd"},
...
function _local_NACE() {
    md_NACE = [	
        {"id": 0, "code": "24540", "shortText": "Buntmetallgießereien", "longText": "Buntmetallgießereien"},
        {"id": 0, "code": "25110", "shortText": "Herstellung von Metallkonstruktionen"},
        {"id": 0, "code": "25120", "shortText": "Herstellung von Ausbauelementen aus Metall"},
        {"id": 0, "code": "25210", "shortText": "Herstellung von Heizkörpern und -kesseln für Zentralheizungen"},
        {"id": 0, "code": "25290", "shortText": "Herstellung von Sammelbehältern, Tanks u. ä. Behältern aus Metall"},
        {"id": 0, "code": "25300", "shortText": "Herstellung von Dampfkesseln (ohne Zentralheizungskessel)"},
        {"id": 0, "code": "25400", "shortText": "Herstellung von Waffen und Munition"},
        {"id": 0, "code": "25500", "shortText": "Herstellung von Schmiede-, Press-, Zieh- und Stanzteilen, gewalzten Ringen und pulvermetallurgischen Erzeugnissen"},
        {"id": 0, "code": "25501", "shortText": "Herstellung von Freiformschmiedestücken"},
        ...
        {"id": 0, "code": "26119", "shortText": "Herstellung von sonstigen elektronischen Bauelementen"},
        {"id": 0, "code": "26120", "shortText": "Herstellung von bestückten Leiterplatten"},
        {"id": 0, "code": "26200", "shortText": "Herstellung von Datenverarbeitungsgeräten und peripheren Geräten"},
        {"id": 0, "code": "05110", "shortText": "xb fgnfgmg mfg Herstellung von Metallkonstruktionen"},
        {"id": 0, "code": "26510", "shortText": "Herstellung von Mess-, Kontroll-, Navigations- u. ä. Instrumenten und Vorrichtungen"}
    ];
}
function _local_EmissionControlSystems() {
    md_EmissionControlSystems = [
        ...
        {"id": 7, "ident": "851", "shortText": "Adsorber/Mechanische Flüss.- Abscheidung"},
        {"id": 8, "ident": "850", "shortText": "Adsorber/Mechanische Trockenabscheidung"},
        {"id": 0, "ident": "857", "shortText": "Adsorber/Oxidations-, Reduktionsverfahren"},
        ...
        {"id": 0, "ident": "753", "shortText": "Biofilter Landwirtschaft (Abscheidegrad Staub 80%)"},
