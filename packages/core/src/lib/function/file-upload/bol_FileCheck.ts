 ...
 */
export function bol_FileCheck(fieldname: string): number {
  ...
  for (let i = 0; i < fileCount; i++) {
    ...
    if (
      (InitForm.bolSettings.FileMaxSize as number) <
      fieldSize / 1024 / 1024
    ) {
  ...
  if ((InitForm.bolSettings.FileMaxSize as number) < fSizes / 1024 / 1024)
    myError = 5;

  if (myError === 0) {
    // Call the appropriate function to update files (replace with the actual function)
    InitForm.bolSettings.UpdateFiles(fieldname, field?.files?.[0].name, fSizes);
  }
